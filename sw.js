// Generated by node scripts/build-pwa.cjs. Do not edit.
const REVISION = '77cba5a48396914d';
...
self.addEventListener('fetch', event => {
  ...
  let key = url.href;
  if (request.mode === 'navigate' && (key === BASE || key === home)) key = home;
  if (!urls.has(key)) return; // No HTML fallback for missing assets or other apps.
  event.respondWith((async () => {
    const cache = await caches.open(CACHE);
    const cached = await cache.match(key);
    if (cached) return cached;
    // A missing entry can indicate browser storage eviction. Do not hide failure.

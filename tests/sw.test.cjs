...
function worker({cached, storageError = false, scope = base} = {}) {
  const handlers = {}, added = [], lookups = [], network = [];
  const cache = {
    addAll: async requests => added.push(...requests.map(r => r.url)),
    match: async key => { lookups.push(key); return cached; }
  };
  vm.runInNewContext(source, {
    URL, Request, Response,
    self: {registration: {scope}, addEventListener: (type, handler) => handlers[type] = handler},
    caches: {open: async () => {if (storageError) throw Error('Storage unavailable'); return cache;}, delete: async () => true},
...
async function navigate(w, url) {
  let result;
  w.handlers.fetch({request: {url, method: 'GET', mode: 'navigate', redirect: 'manual'}, respondWith: value => result = value});
  return result;
}
...
test('redirected cached HTML becomes a navigation-safe response', async () => {
  const cached = new Response('<html>app</html>', {headers: {'Content-Type': 'text/html', 'Content-Security-Policy': "default-src 'self'"}});

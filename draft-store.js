...
window.TutorDraftStore = class TutorDraftStore {
  constructor(scope) { this.name = 'vinee-drafts:' + scope; }
  async open() {
    this.db = await new Promise((resolve, reject) => {
      ...
      request.onblocked = () => reject(new Error('Close other TutorBoard windows to enable local saving.'));
    });
  ...
  transaction(mode, operation) {
    return new Promise((resolve, reject) => {
      ...
      tx.onabort = tx.onerror = () => reject(tx.error || request.error || new Error('Local saving failed.'));
    });
  }
  list() {
    // Read metadata one record at a time; do not retain every lesson's images.
    return new Promise((resolve,reject)=>{
      const tx=this.db.transaction('drafts','readonly'),items=[];
      const request=tx.objectStore('drafts').openCursor();
      request.onsuccess=()=>{
        const cursor=request.result;if(!cursor)return;
        const record=cursor.value;
        items.push({id:record.id,savedAt:record.savedAt,title:record.snapshot?.lesson?.title||'Untitled lesson',pageCount:record.snapshot?.lesson?.pages?.length||0});
        cursor.continue();
      };
